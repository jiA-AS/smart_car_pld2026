// ============================================================================
// fpga_link.h —— ESP32 ↔ FPGA 双向通信模块（V1.2 上行 + V1.1 下行）
//
// 上行：100Hz 发送 35 字节传感器帧（编码器+IMU+显示模式 disp_mode）
...
public:
    ...
    // [V1.2] dispMode：遥控器左拨杆 s1 映射的显示模式（0=视角 1=轨迹 2=调试）
    void send(const int32_t encTicks[4], uint8_t dispMode);
    ...
    // 上行帧原始字节
    const uint8_t* txRaw() const { return lastTxRaw_; }           // 上行原始帧 35 字节
