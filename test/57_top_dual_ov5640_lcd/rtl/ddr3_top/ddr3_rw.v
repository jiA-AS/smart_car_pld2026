...
// File name:           ddr3_rw
// Created by:          正点原子
// Created date:        2025年10月25日13:38:00
// Version:             V1.0
...
module ddr3_rw(  
    ...
    input           wr_clk_2             ,  //输入源2时钟 
    input           wr_clk_1             ,  //输入源1时钟  
    ...
    output          rfifo_wren_2         ,  //读端口FIFO2中的写使能 
...
always @(posedge ui_clk or negedge rst_n)  begin
    if(~rst_n || rd_rst)begin
        rfifo_data_en_2 <= 1'b0;    
    end   
    else begin
        if(state_cnt == DDR3_DONE)
           rfifo_data_en_2 <= 1'b0;
        else if(state_cnt == READ_2 ) 
           rfifo_data_en_2 <= 1'b1; 
        else
    ...
    if(~rst_n || rd_rst )begin
       data_valid_cnt <= 8'd0;    
        ...
        if(state_cnt == DDR3_DONE ) 
           ...
           data_valid_cnt <= data_valid_cnt + 8'd1;
        else
...
always @(posedge ui_clk or negedge rst_n)  begin
    if(~rst_n)
        raddr_rst_h_cnt <= 11'd0;
    else if(raddr_rst_h)
        if(raddr_rst_h_cnt >= 1000)
            raddr_rst_h_cnt <= raddr_rst_h_cnt; 
        else            
            raddr_rst_h_cnt <= raddr_rst_h_cnt + 11'd1;
    else
        raddr_rst_h_cnt <= 11'd0;            
end 
    ...
    if(~rst_n) begin 
        ...
        case(state_cnt)
            ...
            DDR3_DONE:begin  //当wfifo1存储数据超过一次突发长度时，跳到写操作1
                if(wfifo_rcount_1 >= wr_bust_len - 11'd2 )begin  
                    state_cnt <= WRITE_1;                    
                end         //当wfifo2存储数据超过一次突发长度时，跳到写操作2 
                else if(wfifo_rcount_2 >= wr_bust_len - 11'd2 )begin 
                    state_cnt <= WRITE_2;                    
                end                
                else if(raddr_rst_h)begin         //当帧复位到来时，对寄存器进行复位 
                    if(raddr_rst_h_cnt >= 11'd1000 )begin 
                ...
                else if(rfifo_wcount_1 < 11'd5  )begin  //跳到读操作1 
                    state_cnt <= READ_1;                                                 
                end //当rfifo1存储数据少于设定阈值时，并且输入源1已经写入ddr 1帧数据                                    
                else if(rfifo_wcount_2 < 11'd5  )begin  //跳到读操作2 
                    state_cnt <= READ_2;                                                                                        
                end                                                                                                             
                else begin
                    state_cnt <= state_cnt;                      
                ...
                else if(rfifo_wcount_1 < 11'd5 )begin             
                    rd_addr_cnt_1 <= 24'd0;            //计数器清零
                    app_addr_rd_1 <= app_addr_rd_1;    //读地址保持不变
                end //当rfifo1存储数据少于设定阈值时，并且输入源1已经写入ddr 1帧数据 
                else if(rfifo_wcount_2 < 11'd5  )begin             
                ...
                else if(wfifo_rcount_1 >= wr_bust_len - 11'd2 )begin  
            ...
            WRITE_1:   begin 
                if((wr_addr_cnt_1 == (wr_bust_len - 24'd1)) && 
                   (app_rdy && app_wdf_rdy))begin        //写到设定的长度跳到等待状态                  
                    state_cnt    <= DDR3_DONE;           //写到设定的长度跳到等待状态               
                    app_addr_wr_1 <= app_addr_wr_1 + 28'd8;   //一次性写进8个数，故加8
                end       
                else if(app_rdy && app_wdf_rdy)begin       //写条件满足
                    wr_addr_cnt_1  <= wr_addr_cnt_1 + 24'd1;//写地址计数器自加
                    app_addr_wr_1  <= app_addr_wr_1 + 28'd8;   //一次性写进8个数，故加8
                end
            ...
            WRITE_2:   begin 
                if((wr_addr_cnt_2 == (wr_bust_len - 24'd1)) && 
                   (app_rdy && app_wdf_rdy))begin         //写到设定的长度跳到等待状态                  
                    state_cnt    <= DDR3_DONE;            //写到设定的长度跳到等待状态               
                    app_addr_wr_2 <= app_addr_wr_2 + 28'd8;   //一次性写进8个数，故加8
                end       
                else if(app_rdy && app_wdf_rdy)begin      //写条件满足
                    wr_addr_cnt_2  <= wr_addr_cnt_2 + 24'd1; //写地址计数器自加
                    app_addr_wr_2  <= app_addr_wr_2 + 28'd8; //一次性写进8个数，故加8
                end
                else begin                              //写条件不满足，保持当前值     
                    wr_addr_cnt_2  <= wr_addr_cnt_2;
            ...
            READ_1:begin                                  //读到设定的地址长度    
                if((rd_addr_cnt_1 == (rd_bust_len - 24'd1)) && app_rdy)begin
                    state_cnt   <= READ_WAIT;             //则跳到空闲状态 
                    app_addr_rd_1 <= app_addr_rd_1 + 28'd8;
                end       
                else if(app_rdy)begin                   //若MIG已经准备好,则开始读
                    rd_addr_cnt_1 <= rd_addr_cnt_1 + 24'd1; //用户地址计数器每次加一
                    app_addr_rd_1 <= app_addr_rd_1 + 28'd8; //一次性读出8个数,DDR3地址加8
                end
            ...
            READ_2:begin                         //读到设定的地址长度    
                if((rd_addr_cnt_2 == (rd_bust_len - 24'd1)) && app_rdy)begin
                    state_cnt   <= READ_WAIT;             //则跳到空闲状态 
                    app_addr_rd_2 <= app_addr_rd_2 + 28'd8;
                end       
                else if(app_rdy)begin                      //若MIG已经准备好,则开始读
                    rd_addr_cnt_2 <= rd_addr_cnt_2 + 24'd1; //用户地址计数器每次加一
                    app_addr_rd_2 <= app_addr_rd_2 + 28'd8; //一次性读出8个数,DDR3地址加8
                end
                else begin                                 //若MIG没准备好,则保持原值
            ...
            READ_WAIT:begin       //计到设定的地址长度    
                if((data_valid_cnt >= rd_bust_len - 8'd1) && app_rd_data_valid)begin
