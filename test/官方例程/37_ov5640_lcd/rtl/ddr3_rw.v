...
// File name:           ddr3_rw
// Created by:          正点原子
// Created date:        2025年11月010日14:20:02
// Version:             V1.0
    ...
    if(~rst_n) begin 
        ...
        case(state_cnt)
            ...
            DDR3_DONE:begin
                ...
                else if(app_addr_rd >= app_addr_rd_max_a - 28'd8)begin  
                        state_cnt   <= DDR3_DONE;
                        rd_addr_cnt <= 10'd0;      
                        app_addr_rd <= app_addr_rd_min_a; 
                        rd_end      <= 1'b1;
                end    //当写到结束地址对寄存器复位
                else if(app_addr_wr >= app_addr_wr_max_a - 28'd8)begin  
                        state_cnt   <= DDR3_DONE;
                        wr_addr_cnt <= 10'd0;      
                        app_addr_wr <= app_addr_wr_min_a; 
                        wr_end      <= 1'b1;
                end                             
                else if(wfifo_rcount >= wr_bust_len_a - 8'd2)begin  
                    state_cnt   <= WRITE;              //跳到写操作
                    wr_addr_cnt <= 10'd0;                       
                    app_addr_wr <= app_addr_wr;      //写地址保持不变
                end
                else if(raddr_rst_h)begin           //当帧复位到来时，对寄存器进行复位 
                    if(raddr_rst_h_cnt >= 11'd1000 && ddr3_read_valid)begin  
            ...
            WRITE:   begin 
                if((wr_addr_cnt == (wr_bust_len_a - 8'd1)) && 
                   (app_rdy && app_wdf_rdy))begin    //写到设定的长度跳到等待状态                  
                    state_cnt   <= DDR3_DONE;       //写到设定的长度跳到等待状态               
                    app_addr_wr <= app_addr_wr + 28'd8;  //一次性写进8个数，故加8
                end       
                else if(app_rdy && app_wdf_rdy)begin   //写条件满足
                    wr_addr_cnt <= wr_addr_cnt + 10'd1;//写地址计数器自加
                    app_addr_wr <= app_addr_wr + 28'd8;   //一次性写进8个数，故加8
                end
            ...
            READ:begin                      //读到设定的地址长度    
                if((rd_addr_cnt == (rd_bust_len_a - 8'd1)) && app_rdy)begin
                    state_cnt   <= DDR3_DONE;          //则跳到空闲状态 
                    app_addr_rd <= app_addr_rd + 28'd8;
                end       
                else if(app_rdy)begin               //若MIG已经准备好,则开始读
                    rd_addr_cnt <= rd_addr_cnt + 10'd1; //用户地址计数器每次加一
                    app_addr_rd <= app_addr_rd + 28'd8; //一次性读出8个数,DDR3地址加8
                end
