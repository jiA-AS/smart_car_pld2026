...
//reg define
reg          uart_rxd_d0;  //uart_rxd延迟一个时钟周期输出的信号
reg          uart_rxd_d1;  //uart_rxd延迟两个时钟周期输出的信号
reg          uart_rxd_d2;  //uart_rxd延迟三个时钟周期输出的信号
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) 
        ...
        if(baud_cnt == BAUD_CNT_MAX - 16'b1) //当波特率计数器计数到一个波特率周期时
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) 
        ...
        if(baud_cnt == BAUD_CNT_MAX/2 - 16'b1) begin  //判断baud_cnt是否计数到数据位的中间
           case(rx_cnt)
