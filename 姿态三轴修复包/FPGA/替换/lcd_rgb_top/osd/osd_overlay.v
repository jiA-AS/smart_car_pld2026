...
module osd_overlay #(
    ...
    input  wire signed [15:0] yaw_cdeg,  // [V4] 航向角 0.01°，±180°（attitude_cf V4）
...
reg [15:0] pch_a, pch_b, rol_a, rol_b, spd_a, spd_b, yaw_a, yaw_b;
reg [ 7:0] ttc_a, ttc_b;
...
// [V4] yaw 分解（±180.0°）
wire [15:0] y_abs  = yaw_b[15] ? (~yaw_b + 16'd1) : yaw_b;
wire [ 7:0] y_deg  = y_abs / 100;                 // 0~180
wire [ 3:0] y_frac = (y_abs % 100) / 10;
...
localparam [12:0] L2_W = 13'd248;   // [V4] +Y:±123.4（8字符）
    ...
    // ---------- [V3] 姿态行 "姿态 P:+12.3 R:-01.2 deg" ----------
        ...
        if (x_rel2 < 11'd32) begin
            ...
            case (col2)
                ...
                6'd15: ch = 8'h20;
                6'd16: ch = 8'h59;  6'd17: ch = 8'h3A;                     // Y:
                6'd18: ch = yaw_b[15] ? 8'h2D : 8'h2B;
                6'd19: ch = dec2ch(y_deg / 100);
                6'd20: ch = dec2ch((y_deg / 10) % 10);
                6'd21: ch = dec2ch(y_deg % 10);
                6'd22: ch = 8'h2E;                                         // .
                6'd23: ch = dec2ch(y_frac);
                default: ch = 8'h20;
            endcase
        end
    end
