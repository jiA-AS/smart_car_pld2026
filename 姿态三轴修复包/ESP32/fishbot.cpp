 ...
 * [V5] ESP 端三轴姿态解算（pitch/roll/yaw，与 FPGA attitude_cf V4 同算法）：
 *   上电约5s静止校准三轴零偏 + 静止时 ZUPT 温漂跟踪（治"角度一直飘"），
 *   通过 display.updateAttitude() 送 OLED 姿态页（s1 中档），单位 deg。
...
void loop_fishbot_control()
{
    ...
    // ================= PLD2026: OLED 通信调试数据更新 =================
    {
        ...
        // ================= PLD2026 V5: 三轴姿态解算（pitch/roll/yaw，送 OLED 姿态页） =================
        // 与 FPGA attitude_cf V4 同算法：陀螺积分 + 加速度重力角，一阶互补 α=0.98。
        // 陀螺灵敏度按芯片选择：MPU6050=131 LSB/dps，LSM6DS3≈114（±250dps 档）。
        // [V5] 治"一直飘"：①校准 200→500 次（约5s，上电务必静止）；
        //      ②三轴零偏 + 静止时 ZUPT 慢速自适应（τ≈20s，跟踪温漂）；
        //      ③新增 yaw（gyro_z 开环积分，±180° 回绕；无磁力计，分钟级缓慢漂为物理特性）。
        {
            static float pitch = 0.0f, roll = 0.0f, yaw = 0.0f;
            static float bias_x = 0.0f, bias_y = 0.0f, bias_z = 0.0f;
            static uint32_t cal_n = 0, last_ms = 0;
            static double  cal_sx = 0, cal_sy = 0, cal_sz = 0;
            const float lsb = (fpga.imuChip() == 2) ? 114.0f : 131.0f;
            uint32_t now2 = millis();
            float dt = (last_ms == 0) ? 0.01f : (now2 - last_ms) * 0.001f;
            last_ms = now2;
            if (dt > 0.05f) dt = 0.05f;            // 防卡顿后大步长
            // 上电约 5s 静止自校准三轴零偏（与 FPGA 一致：上电请勿动车）
            if (cal_n < 500) {
                cal_sx += gyro[0];  cal_sy += gyro[1];  cal_sz += gyro[2];  cal_n++;
                if (cal_n == 500) {
                    bias_x = cal_sx / 500.0;  bias_y = cal_sy / 500.0;  bias_z = cal_sz / 500.0;
                }
            } else {
                float gxd = gyro[0] - bias_x;
                float gyd = gyro[1] - bias_y;
                float gzd = gyro[2] - bias_z;
                // 静止检测（三轴角速度全 <4 LSB≈0.03°/s）→ 零偏 ZUPT 自适应（τ≈20s）
                bool still = (fabsf(gxd) < 4.0f) && (fabsf(gyd) < 4.0f) && (fabsf(gzd) < 4.0f);
                if (still) {
                    float k = dt / 20.0f;
                    bias_x += gxd * k;  bias_y += gyd * k;  bias_z += gzd * k;
                }
                ...
                // 陀螺积分 + 互补融合（pitch/roll）；yaw 纯开环积分 + 回绕
                pitch += gyd / lsb * dt;
                roll  += gxd / lsb * dt;
                yaw   += gzd / lsb * dt;
                if (yaw > 180.0f) yaw -= 360.0f; else if (yaw < -180.0f) yaw += 360.0f;
