// ============================================================================
// tracker_ctrl.v -- 绿灯追踪控制器 V4（三模式 + TTC预警联动）
//
// V4 改动：新增 warn_level 输入（ttc_warn 输出）：
//   level=1（TTC<3s）→ MONO/MATCH 速度减半（含转向分量，保守减速）
//   level=2（TTC<1.5s）→ MONO/MATCH 速度清零（防撞停车，mode 不变）
//
...
module tracker_ctrl #(
    ...
    // ---- [V4] TTC 预警（ttc_warn，50M 域） ----
    input  wire [ 1:0] warn_level,  // 0=正常 1=半速 2=停车
    // ---- V1.1 指令输出（%） ----
...
// 左右轮合成 + 限幅 [-100, +100]（%）
// 限幅函数：返回 16bit 补码（赋给 signed 信号使用，bit pattern 一致）
function [15:0] clamp100;
    input signed [15:0] v;
    begin
        if (v > 16'sd100)       clamp100 = 16'd100;
        else if (v < -16'sd100) clamp100 = 16'hFF9C;   // -100 补码
        else                    clamp100 = v;
    end
endfunction
...
// [V4] TTC 预警联动：level1 半速、level2 停车（MONO/MATCH 通用）
// 预警缩放：返回 16bit 补码
function [15:0] warn_scale;
    input signed [15:0] v;
    input [1:0] lvl;
    begin
        case (lvl)
            2'd2:    warn_scale = 16'd0;                  // 红警：停车
            2'd1:    warn_scale = v >>> 1;                // 黄警：半速
            default: warn_scale = v;
        endcase
    end
endfunction
wire signed [15:0] mono_lw = warn_scale(mono_l, warn_level);
wire signed [15:0] mono_rw = warn_scale(mono_r, warn_level);
wire signed [15:0] mtch_lw = warn_scale(mtch_l, warn_level);
wire signed [15:0] mtch_rw = warn_scale(mtch_r, warn_level);
