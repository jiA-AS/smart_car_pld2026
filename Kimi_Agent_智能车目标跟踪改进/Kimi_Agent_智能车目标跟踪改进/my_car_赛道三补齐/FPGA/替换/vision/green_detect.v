// ============================================================================
// green_detect.v -- 绿灯识别（像素流水线，零帧存） V3
//
// V3 改动：TH_G/TH_RB 由编译期 parameter 改为运行时输入端口 th_g/th_rb，
//   由 mode_switch 驱动（正常/夜间档位在线切换），参数保留作默认参考。
//
...
module green_detect #(
    ...
    // ---- [V3] 运行时阈值（mode_switch 输出；原 TH_G/TH_RB 参数不再使用） ----
    input  wire [ 5:0] th_g,          // 绿分量下限（0~63）
    input  wire [ 4:0] th_rb,         // 红/蓝分量上限（0~31）
    // ---- 检测结果（帧结束更新，保持到下一帧） ----
