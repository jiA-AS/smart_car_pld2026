// ============================================================================
// v1_decode.v —�?? V1.0 传感器协议解码器（自包含，替�? 45 工程�? packet_decode�?
// 帧格式（34 字节，小端）�?
//   AA 55 | 01 | ts(2) | enc0~3(4×4) | gyro x/y/z(3×2) | acc x/y/z(3×2) | CRC8
//   CRC8/SMBUS(0x07) 覆盖�? 2~32 字节（版本字节起�? 31 字节，不含帧头）
// 输出：解包寄存器（保持到下一好帧覆盖�?+ parse_done 脉冲（仅好帧产生�?
// ============================================================================
module v1_decode (
    input  wire               clk,
    input  wire               rst_n,
    input  wire       [ 7:0]  rx_data,      // �? sl_uart_rx
    input  wire               rx_done,
    ...
    output reg                parse_done,    // 好帧单拍脉冲（下�? frame_valid 用这个）
    output reg        [ 7:0]  parse_result   // 0x00=好帧 0xE3=CRC�?
);
...
reg [ 1:0] state = S_H1;
reg [ 5:0] byte_cnt = 0;        // 0~30：共 31 个数据字�?
reg [ 7:0] crc = 0;
