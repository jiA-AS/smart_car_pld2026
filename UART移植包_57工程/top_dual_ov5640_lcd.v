...
module top_dual_ov5640_lcd(    
    ...
    //UART接口（传感器链，接 ATK-MODULE 插座 J1）
    input                 uart_rxd      ,   //ESP32 -> FPGA（V1.0 传感器帧）J1:UART2_RXD=E13
    output                uart_txd          //FPGA -> ESP32/PC（V1.1 下行帧）J1:UART2_TXD=D16（冒烟测试期可约束到 D17 用电脑 USB 串口观察）
    );                                 
